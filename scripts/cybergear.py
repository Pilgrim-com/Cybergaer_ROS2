...
class CyberGear:
    ...
    def type8(self, id, index, data, width='B', echo=False):
        """
        Write a parameter to the parameter table.

        Parameters:
        id (int): The motor ID.
        index (int): The parameter index/code.
        data: The data to write.
        width (str): Format specifier ('B' - uint8, 'h' - int16, 'H' - uint16, 'l' - int32, 'L' - uint32, 'f' - float).
        echo (bool): Whether to print debug messages.

        Returns:
        The response data if successful, otherwise None.
        """
        # Create a mapping from width to type code
        type_codes = {
            'B': 0x00,  # uint8
            'H': 0x02,  # uint16
            'h': 0x03,  # int16
        ...
        if width not in type_codes:
            return None

        type_code = type_codes[width]

        # Pack the data according to the specified width
        packed_data = bytes()
        if width == 'B':
            packed_data = bytes(pack("<Bxxx", data))
        elif width == 'H':
            packed_data = bytes(pack("<Hxx", data))
        elif width == 'h':
            packed_data = bytes(pack("<hxx", data))
        elif width == 'L':
            packed_data = bytes(pack("<L", data))
        elif width == 'l':
            packed_data = bytes(pack("<l", data))
        elif width == 'f':
            packed_data = bytes(pack("<f", data))

        # Prepare the message data
        msg_data = bytes(pack("<H", index)) + bytes([type_code, 0x00]) + packed_data

        # Send the message
        if self.__send(8, id, self.__myid, msg_data, echo=echo):
    ...
    def set_param_value_type8(self, id, name_or_code: str | int, val, echo=False) -> bool:
        """
        Set a parameter value using type8 command (parameter table write).

        Parameters:
        id (int): The motor ID.
        name_or_code (str | int): The parameter name or code.
        ...
        if type(name_or_code) is str:
            ...
            if f:
                return self.type8(id, f["code"], val, f["format"], echo=echo) is not None
        else:
