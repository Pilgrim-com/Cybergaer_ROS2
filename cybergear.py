...
class CyberGear:
    ...
    def __send(
        self, cmd: int, id: int, id_opt: int, data: bytes = bytes(8), echo=False
    ) -> bool:
        if id <= 0x7F:
            try:
                msg = can.Message(
                    arbitration_id=self.__makeaid(cmd, id, id_opt)[0],
                    data=data,
                    is_extended_id=True,
                )
                ...
                if echo:
                    print(
                        f"TX: id=${msg.arbitration_id:08x} data=",
            ...
            try:
                ...
                if r != None:
                    if r.is_rx:
                        if echo:
                            print(
                                f"RX: id=${r.arbitration_id:08x} data=",
                                ":".join(format(x, "02x") for x in r.data),
                            )
                        typ = (r.arbitration_id >> 24) & 0xFF
                        if typ == 21:
                            self.__alarm.append(
                                (
                                    (r.arbitration_id >> 8) & 0xFF,
                                    r.timestamp,
                                    tuple(*iter_unpack("<Q", bytes(r.data)))[0],
                                )
                            )
                        elif typ == 22:
                            pass
    ...
    def type1(
        self, id: int, torque: int, angle: int, speed: int, Kp: int, Kd: int, echo=False
    ) -> tuple:
        """
        controle mode
        ...
        with self.__lock:
            if self.__send(
                1, id, torque, bytes(pack(">HHHH", angle, speed, Kp, Kd)), echo=echo
            ):
                r = self.__recv(
                    matchaid=self.__makeaid(2, self.__myid, id, 0xFF00FFFF), echo=echo
                )
                if r != None:
                    return ((r.arbitration_id & 0xFF0000) >> 16,) + tuple(
                        *iter_unpack(">HHHH", bytes(r))
                    )
    ...
    def type4(self, id: int, fault=False, echo=False) -> tuple:
        ...
        with self.__lock:
            if self.__send(
                4, id, self.__myid, (1 if fault else 0, 0, 0, 0, 0, 0, 0, 0), echo=echo
            ):
                r = self.__recv(
                    matchaid=self.__makeaid(2, self.__myid, id, 0xFF00FFFF), echo=echo
                )
            ...
            if self.__send(9, id, self.__myid, bytes(pack("<Hxxxxxx", index)), echo):
                ...
                while lcnt >= cnt:
                    if time.time() > t:
                        return
                    r = self.__recv(
                        matchaid=self.__makeaid(9, self.__myid, id), echo=echo
                    )
                    if r != None:
    ...
    def type17(self, id: int, index: int, width="B", echo=False) -> None | int | float:
        ...
        with self.__lock:
            if self.__send(
                17, id, self.__myid, bytes(pack("<Hxxxxxx", index)), echo=echo
            ):
                r = self.__recv(
                    matchaid=self.__makeaid(17, self.__myid, id, 0xFF00FFFF), echo=echo
                )
                if r != None:
                    match width:
                        case "B":
    ...
    def type18(
        self, id: int, index: int, data: int | float, width="B", echo=False
    ) -> tuple:
        """
        write param
    ...
    def type19(self, id: int, echo=False) -> bytes:
        ...
        with self.__lock:
            if self.__send(
                ...
                id,
                self.__myid,
                (0xC4, 0x7F, 0x31, 0x31, 0x30, 0x33, 0x31, 0x04),
                echo=echo,
            ):
                s = bytes()
                t = time.time() + 5.0
    ...
    def type22(self, id: int, baud: int, echo=False) -> int:
        ...
        if baud >= 1 and baud <= 4:
            with self.__lock:
                if self.__send(
                    22, id, self.__myid, (baud, 0, 0, 0, 0, 0, 0, 0), echo=echo
                ):
                    r = self.__recv(matchaid=self.__makeaid(0, 0xFE, id), echo=echo)
                    if r != None:
                        return tuple(*iter_unpack("<Q", bytes(r.data)))[0]
    ...
    def stop_motor(
        self, ids: int | list | tuple, fault=False, echo=False
    ) -> bool | tuple:
        """
        stop motor
        """
                ...
                if r[1] != None:
                    ...
                    if show:
                        match r[0]["format"]:
                            case "B":
                                print(
                                    f'${r[0]["code"]:04x} {r[0]["name"]:15}: ${r[1]:02x} {r[0]["unit"]}'
                                )
                            case "f":
                                print(
                                    f'${r[0]["code"]:04x} {r[0]["name"]:15}: {r[1]:.3f} {r[0]["unit"]}'
                                )
                            case _:
                                print(
                                    f'${r[0]["code"]:04x} {r[0]["name"]:15}: {r[1]} {r[0]["unit"]}'
                                )
        return result
...
if __name__ == "__main__":
    ...
    with CyberGear(
      ...
      """
            ...
            for p in (
                tuple(range(32767, 65535, 1024))
                + tuple(range(65535, 0, -1024))
                + tuple(range(0, 32767, 1024))
            ):
                cg.type1(target_id, 32767, p, 32767, 50, 100)
                t = time.time() + 500 / 1000.0
                while t > time.time():
                    print(
                        f'\r<{target_id}> pos:{p:>6d} {cg.get_items_as_str(target_id,("VBUS", "boardTemp", "iq", "mechPos", "mechVel", "rotation"))}\033[K',
                        end="",
                    )
                    a = cg.alarm
                    if a != []:
            ...
            for p in (
                ...
                while t > time.time():
                    print(
                        f'\r<{target_id}> tpos:{pos:>6.2f} {cg.get_items_as_str(target_id,("VBUS", "boardTemp", "iq", "mechPos", "mechVel", "rotation"))}\033[K',
                        end="",
                    )
                    a = cg.alarm
                    if a != []:
            ...
            for s in (
                ...
                while t > time.time():
                    print(
                        f'\r<{target_id}> tvel:{s:>6.2f} {cg.get_items_as_str(target_id,("VBUS", "boardTemp", "iq", "mechPos", "mechVel", "rotation"))}\033[K',
                        end="",
                    )
                    a = cg.alarm
                    if a != []:
            ...
            for c in (
                ...
                while t > time.time():
                    print(
                        f'\r<{target_id}> tcur:{c:>6.2f} {cg.get_items_as_str(target_id,("VBUS", "boardTemp", "iq", "mechPos", "mechVel", "rotation"))}\033[K',
                        end="",
                    )
                    a = cg.alarm
                    if a != []:
